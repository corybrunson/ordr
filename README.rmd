...
```{r attach, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, results=FALSE}
library(ordr)
```
...
The `master` branch exists only as the required source from which to build [the website](https://corybrunson.github.io/ordr/), and will be discontinued once GitHub Pages enables building from the default branch.
