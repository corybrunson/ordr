...
When variables are represented in standard coordinates, as typically in PCA, their rules can be rescaled to yield a prediction biplot:[^prediction]

[^prediction]: This is an experimental feature only available for linear methods, namely eigendecomposition, singular value decomposition, and principal components analysis.
...
The auxiliary package [**ordr.extra**](https://github.com/corybrunson/ordr.extra) provides accessor methods for several additional ordination models---and has room for several more!
