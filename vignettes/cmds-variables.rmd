...
[The six QS rankings](https://www.topuniversities.com/qs-world-university-rankings/methodology) are
...
The **corrplot** package provides versatile and high-quality heatmap correlation plots, and the default style is used below.
...
eigen(corr) %>%
  ...
  mutate(name = rownames(corr)) %>%
  mutate(metric = stringr::str_remove(name, "rk_")) ->
  c_eigen
c_eigen %>%
  ...
  geom_unit_circle() +
  geom_vector() +
  geom_text_radiate(aes(label = metric)) +
  scale_x_reverse(expand = expansion(add = .6)) +
  scale_y_continuous(expand = expansion(add = .4)) +
  ggtitle("Kendall correlations between university rankings",
          "CMDS correlation vector plot")
```
...
This last point means that observations made from a correlation vector plot can be validated from the CMDS coordinates. In particular, we can examine the variables' loadings onto the third principal coordinate, and we can check whether the reputational rankings are aligned or misaligned along it.

```{r}
c_eigen %>%
  select(-row, -name, -metric)
```
...
c_eigen %>%
  ...
  ggtitle("Kendall correlations between university rankings",
          "CMDS correlation vector plot, second & third principal coordinates")
```
