...
test_that("matrix factor stats don't throw errors", {
  ...
  p <- ggbiplot(as_tbl_ord(princomp(swiss)))
  
  # matrix factor stats
  expect_no_error(ggplot_build(p + stat_rows(geom = "point")))
  expect_no_error(ggplot_build(p + stat_cols(geom = "vector")))
  
  # with additional parameters
  expect_no_error(ggplot_build(p + stat_rows(geom = "point", elements = "score")))
  expect_no_error(ggplot_build(p + stat_cols(geom = "vector", subset = 1:2)))
