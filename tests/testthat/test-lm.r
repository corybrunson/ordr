...
test_that("'as_bibble()' coerces 'lm' objects", {
  expect_true(is.bibble(as_bibble(fit_lm0)))
  expect_true(is.bibble(as_bibble(fit_lm1)))
  expect_true(is.bibble(as_bibble(fit_lm2)))
  expect_true(is.bibble(as_bibble(fit_lm3)))
})
...
fit_glm3 <- glm(as.matrix(bioenv[, "d"]) ~
                  ...
                  as.matrix(bioenv[, "y"]),
                family = poisson)
test_that("'as_bibble()' coerces 'glm' objects", {
  expect_true(is.bibble(as_bibble(fit_glm0)))
  expect_true(is.bibble(as_bibble(fit_glm1)))
  expect_true(is.bibble(as_bibble(fit_glm2)))
  expect_true(is.bibble(as_bibble(fit_glm3)))
})
...
fit_mlm3 <- lm(as.matrix(bioenv[, c("a", "b", "c", "d", "e")]) ~
                 as.matrix(bioenv[, "x"]) +
                 as.matrix(bioenv[, "y"]))
test_that("'as_bibble()' coerces 'mlm' objects", {
  expect_true(is.bibble(as_bibble(fit_mlm0)))
  expect_true(is.bibble(as_bibble(fit_mlm1)))
  expect_true(is.bibble(as_bibble(fit_mlm2)))
  expect_true(is.bibble(as_bibble(fit_mlm3)))
})
