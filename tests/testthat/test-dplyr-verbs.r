...
test_that("`bind_cols_*()` appends a column only of the correct length", {
  expect_equal(
    ncol(fortify(bind_cols_u(pca, iris[, 5, drop = FALSE]), .matrix = "u")),
    4 + 1 + 1
  )
  expect_equal(
    ncol(fortify(bind_cols_u(pca, species = iris[[5]]), .matrix = "u")),
    4 + 1 + 1
  )
...
test_that("`select()` applies to both augmented and annotated variables", {
  expect_false("Species" %in% names(fortify(select_u(pca, -Species))))
  expect_true("species" %in% names(fortify(select_u(pca, species = Species))))
})
